# app.py - single-file PNP roster + admin + live Roblox avatar (username-based)
import os
import json
import time
import threading
...
AVATAR_TTL = int(os.getenv("AVATAR_TTL", 60 * 60))  # 1 hour by default
...
# PNP ranks (lowest -> highest) EXACT order you provided
PNP_RANKS = [
...
# ---------------- Roblox helpers (username -> userid -> headshot) ----------------
...
def get_roblox_avatar(username, size=AVATAR_SIZE):
    # return cached first (cached may be None)
...
def login():
    if request.method == "POST":
        ...
        # show a small invalid message inline (still single-file)
        return render_template_string(
    ...
    # GET -> show a simple login page embedded
    return render_template_string(
        "<div style='max-width:420px;margin:40px auto;padding:20px;background:#0f0f0f;border-radius:8px;color:#fff'>"
        "<h2>Admin Login</h2>"
        "<form method='post'>"
...
if __name__ == "__main__":
    ...
    # Use gunicorn on Render: `gunicorn app:app`
    app.run(host="0.0.0.0", port=PORT, debug=False)  <meta charset="utf-8">
  <title>PROBISYA — PNP Roster</title>
  <meta name="viewport" content="width=device-width,initial-scale=1">
  <style>
    :root{--bg:#070708;--card:#111213;--muted:rgba(255,255,255,0.6);--accent:#ffd54b}
